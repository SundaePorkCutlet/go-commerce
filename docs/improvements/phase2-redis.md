...
### Q9. "Redis는 메모리 DB인데, 재시작하면 데이터가 다 사라지는 게 정상 아닌가요?"

> **런타임**에서는 맞게 **메모리**를 씁니다. 다만 Redis는 **RDB 스냅샷·AOF**로 디스크에 남길 수 있고, Docker로 쓸 때 **`/data`에 볼륨**을 붙이면 그 파일이 유지됩니다. 기본 설정에서는 **RDB(`save` 규칙)는 켜져 있는 경우가 많고, AOF는 꺼져 있는 경우가 많습니다.** 그래서 “무조건 날아간다”기보다 **설정·볼륨·종료 방식**에 따라 달라진다고 답하는 것이 정확합니다. `docker compose down -v` 로 볼륨을 지우면 의도적으로 전부 삭제됩니다.
