...
- **Kubernetes 로컬 배포 검증**: [k8s/README.md](k8s/README.md)
...
| 로그 전송 | Promtail (서비스별) |
| 로컬 오케스트레이션 | Docker Compose, Kubernetes(kind 로컬 배포) |
...
### 2-1. 로컬 Kubernetes 배포 검증 (kind)

클라우드 비용 없이 Kubernetes 배포 경험을 만들기 위해 `k8s/` 디렉토리에 kind 기반 로컬 배포 문서를 추가했습니다.

- 실행/검증 계획: [k8s/README.md](k8s/README.md)
- 운영 노트: [k8s/STUDY.md](k8s/STUDY.md)
...
├── docker-compose.yml
├── README.md
├── k8s/                     # kind 기반 Kubernetes 로컬 배포 검증/manifest 작업 공간
...
프론트엔드는 공개용 **Go Commerce Engineering Portfolio** 첫 화면과 개발용 콘솔 페이지를 함께 제공합니다. 첫 화면은 Saga/Outbox/Kubernetes/Observability 검증 자료를 포트폴리오형 랜딩 페이지로 보여주며, **MongoDB** 탭(`MongoPage`) 같은 Dev Console 페이지에서는 실제 API를 호출합니다. Docker 환경에서는 **포트 `3001`**(compose `frontend` 서비스)에서 동일 UI를 제공합니다.
...
| Phase | 주제 | 핵심 키워드 |
|-------|------|------------|
| 0 | Engineering Portfolio + Dev Console | React, Vite, CORS Proxy |
